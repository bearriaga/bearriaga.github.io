    ...
    <div>
        ...
        <v-card>
            ...
            <v-card-text>
                <template v-if="$signedIn">
                    <v-select label="Parties"
                        ...
                        <template v-slot:append-outer>
                            <v-btn color="success" @click="setCampaign">
                                <v-icon>
                    ...
                    <v-row>
                        <v-col class="text-center">
                            <v-btn color="primary" @click="saveCampaignPopup">
                                Save Party
                                <v-icon>
                                    mdi-content-save
                                </v-icon>
                        ...
                        <v-col class="text-center">
                            <v-btn color="primary" @click="clearCampaign">
                                New Party
                                <v-icon>
                ...
                <MinionSection :array-name="'party'"
                               :clear-character="clearCharacter"
                               :minions="party"
                               :panel-prop="minionPanel"
                               :key="updateMinions"
        ...
        <v-dialog v-model="generalDialog.show" width="500">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <div v-if="generalDialog.buttonType == 'saveToDatabase'">
                        <v-text-field label="Campaign Name" v-model="campaign.name"></v-text-field>
                    </div>
                </v-card-text>
                ...
                <v-card-actions class="justify-end">
                    <v-btn v-if="generalDialog.buttonText && generalDialog.buttonType" :color="generalDialog.buttonColor" 
                           @click="generalDialogFunction">{{generalDialog.buttonText}}</v-btn>
...
<script>
    ...
    export default {
        ...
        created() {
            if (this.$signedIn)
                this.loadCampaignsFromFirebase()
        },
        data() {
            return {
                campaign: {
        ...
        methods: {
            ...
            generalDialogFunction() {
                if (this.generalDialog.buttonType == 'saveToDatabase')
                    this.saveCampaignToFirebase()
            },
            async loadCampaignsFromFirebase() {
                this.campaigns = []

                const q = query(collection(db, 'campaigns'), where('user', '==', this.$userData.email))
            ...
            moveEntry(object) {
                ...
                if (object.index == 0 && object.direction == 'up')
                    targetIndex = +this[object.arrayName].length - 1
                else if (object.index == (+this[object.arrayName].length - 1) && object.direction == 'down')
                    targetIndex = 0
                else
            ...
            async saveCampaignToFirebase() {
                ...
                let campaign = JSON.parse(JSON.stringify(this.campaign))
                campaign.user = this.$userData.email
                campaign.characters = this.party
                await setDoc(doc(db, 'campaigns', campaign.id), campaign)                

                if (type == 'add') {
                    this.campaigns.push(campaign)
                    this.showSnackbar('Campaign Saved to Database')
                }
                else if (type == 'update') {
                    let index = this.campaigns.findIndex(x => { return x.id == campaign.id })
                    if (index > -1)
                        this.campaign[index] = campaign
                    this.showSnackbar('Campaign Updated in Database')
                }
                this.generalDialog.show = false
            },
            setCampaign() {
                if (this.campaignId) {
                    this.campaign = this.campaigns.find(x => { return x.id == this.campaignId })
                    this.party = this.campaign.characters
                    this.showSnackbar('Loaded Party')
                }
            },
            updateEntry(object) {
